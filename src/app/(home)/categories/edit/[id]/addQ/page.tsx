...
export default function AddQuestionPage() {
  ...
  const [tempQuestionImage, setTempQuestionImage] = useState<string | null>(null); // For cropping
  
  // Answer image states
  const [answerImage, setAnswerImage] = useState<string | null>(null); // Preview
  const [answerImageFile, setAnswerImageFile] = useState<File | null>(null); // File for upload
  const [tempAnswerImage, setTempAnswerImage] = useState<string | null>(null); // For cropping
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend expects questions as form fields: questions[0][text], questions[0][answer], questions[0][points]
      ...
      // Add images if uploaded (for future backend support)
      ...
      await gameAPI.addQuestionsToCategory(Number(id), formData);
      ...
      setError(errorObj?.error || errorObj?.detail || errorObj?.message || "There was an error adding the question");
    } finally {
      setLoading(false);
    }
  };
